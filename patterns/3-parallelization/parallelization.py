...
class CalendarValidation(BaseModel):
    """Check ifs input is a valid calendar request"""
    is_calendar_request: bool = Field(description="Whether this is a calendar request")
    confidence_score: float = Field(description="Confidence score between 0 and 1")


class SecurityCheck(BaseModel):
    """Checks for prompt injection or system manipulation attempts"""
...
async def validate_calendar_request(user_input: str) -> CalendarValidation:
    """Checks if the input is a valid calendar request"""
    client = AsyncClient()
    ...
    result = CalendarValidation.model_validate_json(response.message.content)
    return result


async def check_security(user_input: str) -> SecurityCheck:
    """Checks for potential security risks"""
    client = AsyncClient()
    response: ChatResponse = await client.chat(
        model="llama3.1",
        messages=[
    ...
    result = SecurityCheck.model_validate_json(response.message.content)
    return result
